...
public class CadastroRestauranteIT {
	...
	private Restaurante burgetTopRestaurente;
	...
	public void deveRetornarRespostaEStatusCorretos_QuantoConsultarRestauranteExistente() {
		RestAssured.given().pathParam("restauranteId", burgetTopRestaurente.getId()).accept(ContentType.JSON).when()
				.get("/{restauranteId}").then().statusCode(HttpStatus.OK.value())
				.body("nome", equalTo(burgetTopRestaurente.getNome()));
	}
	...
	private void prepararDados() {
		...
		burgetTopRestaurente = new Restaurante();
		burgetTopRestaurente.setNome("BurgerTop");
		burgetTopRestaurente.setTaxaFrete(new BigDecimal(10));
		burgetTopRestaurente.setCozinha(cozinhaAmericana);
		restauranteRepository.save(burgetTopRestaurente);
